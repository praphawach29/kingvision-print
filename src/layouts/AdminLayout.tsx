...
const NAV_ITEMS = [
  ...
  { path: '/admin/leads', icon: MessageSquare, label: 'LINE Leads' },
...
function SidebarContent({ currentPath, onClose }: SidebarProps) {
  return (
    <>
...
export function AdminLayout() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-kv-orange" />
      </div>
    );
  }
  ...
  const currentLabel = NAV_ITEMS.find(item => item.path === location.pathname)?.label || 'ระบบจัดการหลังบ้าน';

  return (
    <div className="min-h-screen bg-gray-100 flex font-thai overflow-hidden">
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex w-64 bg-kv-navy text-white flex-col shrink-0 z-10">
        <SidebarContent currentPath={location.pathname} onClose={closeMobile} />
      </aside>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        {/* Header */}
        <header className="bg-white h-16 shadow-sm flex items-center justify-between px-4 lg:px-8 shrink-0 relative z-20">
        ...
        {/* Page Content — Suspense here keeps sidebar visible during lazy page load */}
