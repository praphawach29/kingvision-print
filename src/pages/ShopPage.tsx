...
import { ChevronRight, Filter, Grid, List, Star, ShoppingCart, ChevronDown, ChevronUp, X, CheckCircle2, Loader2, ArrowLeft } from 'lucide-react';
...
export function ShopPage() {
  ...
  const categoryParam = searchParams.get('category') || categoryUrlParam;
  ...
  const [priceRange, setPriceRange] = useState(maxPriceParam ? parseInt(maxPriceParam) : 10000);
  ...
  useEffect(() => {
    fetchProducts();
  }, [currentPage, categoryParam, brandParam, minPriceParam, maxPriceParam, isSaleParam, conditionParam, sortByParam]);
  ...
  async function fetchProducts() {
    ...
    try {
      ...
      switch (sortByParam) {
        ...
        default:
          if (!categoryParam) {
            // Prioritize Printer category (starts with 'เ' which is high in Unicode)
            query = query.order('category', { ascending: false }).order('created_at', { ascending: false });
          } else {
                ...
                <div className="mt-4 bg-[#f39c12] p-6 rounded-lg shadow-inner grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="bg-white p-4 rounded-lg shadow-sm">
                    ...
                    <div className="space-y-3">
                      <input 
                        type="range" 
                        className="w-full accent-kv-orange" 
                        min="0" 
                        max="20000" 
                        value={priceRange}
                        onChange={(e) => {
                          const val = e.target.value;
                          setPriceRange(parseInt(val));
                          setMaxPrice(val);
                        }}
                      />
                      <div className="flex items-center gap-2">
                        <input 
                          ...
                          className="w-full p-2 border border-kv-border rounded text-xs" 
                        />
            ...
            <div className="bg-white p-5 rounded-lg shadow-sm border border-kv-border">
              ...
              <ul className="space-y-3 text-sm text-gray-600">
                <Link 
                  ...
                  className={`flex justify-between items-center hover:text-kv-orange cursor-pointer transition-colors ${!categoryParam ? 'text-kv-orange font-bold' : 'text-gray-600'}`}
                >
                  <span>ทั้งหมด</span>
                </Link>
                {dbCategories.map((cat) => (
                  <li key={cat.id} className="space-y-2">
                    ...
                    >
                      <button
                        ...
                        className={`flex flex-col text-left hover:text-kv-orange transition-colors ${categoryParam === cat.name ? 'text-kv-orange font-bold' : 'text-gray-600'}`}
                      >
                        <span>{cat.name}</span>
                        <span className="text-[10px] text-gray-400 font-bold">{categoryCounts[cat.name] || 0} รายการ</span>
                      </button>
                      <div className="text-gray-400 group-hover:text-kv-orange transition-colors">
                            ...
                            categoryProducts[cat.name].map((product) => (
                              <li key={product.id} className="group/item">
                                <Link 
                                  ...
                                  className="text-xs text-gray-500 hover:text-kv-orange cursor-pointer flex flex-col gap-0.5 py-1"
                                >
            ...
            <div className="bg-white p-5 rounded-lg shadow-sm border border-kv-border">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-2">
                  <div className="relative flex-1">
                    ...
                    <input 
                      ...
                      value={minPrice}
                      onChange={(e) => setMinPrice(e.target.value)}
                      className="w-full pl-6 pr-2 py-2 border border-kv-border rounded text-sm outline-none focus:border-kv-orange" 
                    />
                  </div>
                  <span className="text-gray-400">-</span>
                  <div className="relative flex-1">
                ...
                <button 
                  ...
                  className="w-full bg-kv-navy text-white py-2 rounded font-bold hover:bg-kv-navy/90 transition-colors text-sm"
                >
                  กรองราคา
                </button>
              </div>
                ...
                <label className="flex items-center justify-between cursor-pointer group">
                  <div className="flex items-center space-x-3">
                    <input 
                      type="radio" 
                      name="condition"
                      checked={conditionParam === 'มือหนึ่ง'}
                      onChange={() => {
                        const newParams = new URLSearchParams(searchParams);
                        newParams.set('condition', 'มือหนึ่ง');
                        setSearchParams(newParams);
                        setCurrentPage(1);
                      }}
                      className="rounded-full border-gray-300 text-kv-orange focus:ring-kv-orange w-4 h-4" 
                ...
                <label className="flex items-center justify-between cursor-pointer group">
                  <div className="flex items-center space-x-3">
                    <input 
                      type="radio" 
                      name="condition"
                      checked={conditionParam === 'มือสอง'}
                      onChange={() => {
                        const newParams = new URLSearchParams(searchParams);
                        newParams.set('condition', 'มือสอง');
                        setSearchParams(newParams);
                        setCurrentPage(1);
                      }}
                      className="rounded-full border-gray-300 text-kv-orange focus:ring-kv-orange w-4 h-4" 
              ...
              <div className="flex items-center space-x-4">
                <label className="flex items-center space-x-2 cursor-pointer">
                  <input 
                    ...
                    onChange={(e) => {
                      const newParams = new URLSearchParams(searchParams);
                      if (e.target.checked) newParams.set('isSale', 'true');
                      else newParams.delete('isSale');
                      setSearchParams(newParams);
                      setCurrentPage(1);
                    }}
                    className="rounded text-kv-orange focus:ring-kv-orange" 
                  />
                ...
                <select 
                  ...
                  className="p-2 border border-kv-border rounded text-sm outline-none focus:border-kv-orange"
                >
                  <option value="latest">สินค้าใหม่ล่าสุด</option>
                  <option value="popularity">เรียงตามความนิยม</option>
                  <option value="price-asc">เรียงตามราคา: ต่ำไปสูง</option>
